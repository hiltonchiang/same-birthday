...
class SVGLoader extends Loader {
  ...
  parse(text) {
    ...
    function parseNode(node, style) {
      ...
      switch (node.nodeName) {
        ...
        default:
          // console.log('parseNode', node.nodeName)
          break
      }
      if (path) {
        ...
        const P: ShapePathX = { shape: path, userData: { node: node, style: style } }
        paths.push(P)
    ...
    function parsePathNode(node) {
      ...
      //console.log('parsePathNode', d)
      const commands = d.match(/[a-df-z][^a-df-z]*/gi)
      // console.log('parsePathNode, commands', commands, commands.length)
      for (let i = 0, l = commands.length; i < l; i++) {
        ...
        switch (type) {
          case 'M':
            // console.log('parsePathNode', data, 'length', data.length)
            numbers = parseFloats(data, undefined, undefined)
          ...
          case 'Z':
          case 'z':
            if (path !== null && path.currentPath !== null) {
        ...
        // console.log(type, parseFloats(data, undefined, undefined), parseFloats(data, undefined, undefined).length )
    ...
    function parseStyle(node, style) {
      ...
      addStyle('fill', 'fill', undefined)
      addStyle('fill-opacity', 'fillOpacity', clamp)
      addStyle('fill-rule', 'fillRule', undefined)
      addStyle('opacity', 'opacity', clamp)
      addStyle('stroke', 'stroke', undefined)
      addStyle('stroke-opacity', 'strokeOpacity', clamp)
      addStyle('stroke-width', 'strokeWidth', positive)
      addStyle('stroke-linejoin', 'strokeLineJoin', undefined)
      addStyle('stroke-linecap', 'strokeLineCap', undefined)
      addStyle('stroke-miterlimit', 'strokeMiterLimit', positive)
      addStyle('visibility', 'visibility', undefined)
      // console.log('parseStyle', style)
      return style
    }
    ...
    function parseFloats(input: string, flags, stride) {
      ...
      function throwSyntaxError(current, i, partial) {
        const error = new SyntaxError(
          'Unexpected character "' + current + '" at index ' + i + '.' + ' parrtial' + partial
        )
        throw error
      }
      function newNumber() {
    ...
    const parseFloatWithUnits = (string) => {
      let theUnit = 'px'
      ...
      if (node.hasAttribute('transform')) {
        ...
        for (let tIndex = transformsTexts.length - 1; tIndex >= 0; tIndex--) {
          ...
          if (openParPos > 0 && openParPos < closeParPos) {
            ...
            const array = parseFloats(transformText.slice(openParPos + 1), undefined, undefined)
            currentTransform.identity()
    ...
    //
    const paths: ShapePathX[] = [] as ShapePathX[]
    const stylesheets = {}
    const transformStack: Matrix3[] = [] as Matrix3[]
  ...
  static createShapes(shapePath) {
    ...
    function getIntersections(path1, path2) {
      const intersectionsRaw: InterSectionProp[] = [] as InterSectionProp[]
      const intersections: Vector2[] = [] as Vector2[]
    ...
    function getScanlineIntersections(scanline, boundingBox, paths) {
      ...
      const allIntersections: AllInterSectionProp[] = [] as AllInterSectionProp[]
      paths.forEach((path) => {
    ...
    function isHoleTo(simplePath, allPaths, scanlineMinX, scanlineMaxX, _fillRule) {
      ...
      const scanlineIntersections: AllInterSectionProp[] = getScanlineIntersections(
        scanline,
        simplePath.boundingBox,
        allPaths
      ) as AllInterSectionProp[]
      scanlineIntersections.sort((i1, i2) => {
        return i1.point.x - i2.point.x
      })
      const baseIntersections: AllInterSectionProp[] = [] as AllInterSectionProp[]
      const otherIntersections: AllInterSectionProp[] = [] as AllInterSectionProp[]
    ...
    const shapesToReturn: Shape[] = [] as Shape[]
  ...
  static pointsToStrokeWithBuffers(
        ...
        case 'square':
          if (start) {
            ...
            if (joinIsOnLeftSide) {
              ...
              if (uvs[3 * 2 + 1] === 1) tempV2_4.toArray(vertices, 3 * 3)
              else tempV2_3.toArray(vertices, 3 * 3)
              tempV2_4.toArray(vertices, 0 * 3)
            }
          } else {
    ...
    function removeDuplicatedPoints(points: Vector2[]) {
      ...
      const newPoints: Vector2[] = [] as Vector2[]
